...
- **Canonical/export URLs**: When generating absolute URLs for metadata, sitemap, or schema, match `trailingSlash: true` output (`/about/`) instead of non-canonical no-slash variants; file-like routes such as `/sitemap.xml` stay file-like
